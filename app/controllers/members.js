...
const addMultipleItems = (req, res) => {
    try {
        ...
        if (!Array.isArray(newMembers) || newMembers.length === 0) {
            return res.status(400).send({ error: 'Debe enviar un array de miembros' });
        }
...
const updateMultipleItems = (req, res) => {
    try {
        ...
        if (!Array.isArray(updatesArray)) {
            return res.status(400).send({ error: 'Se espera un array de miembros a actualizar' });
        }
        ...
        if (updatedMembers.length === 0) {
            return res.status(404).send({ error: 'No se encontró ningún miembro para actualizar' });
        }
...
module.exports = { getItem, getItems, deleteItem, createItem, updateItem, getByCommunity, addMultipleItems, updateMultipleItems }